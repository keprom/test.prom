...
//        }
    ...
    function Billing()
    {
        ...
        //если не авторизован редирект на логин страницу
        if ($is_login != TRUE) {
            redirect("login/billing");
            die();
        }

        //получаем строку контроллер/метод
        $class_method = $this->uri->segment(1) . '/' . $this->uri->segment(2);

        //прямой редирект на дефолтный контроллер
//        if ($class_method == '/') {
//            redirect("billing");
//        }

        $user_id = $this->session->userdata('id');
        $has_permission = $this->has_permission($class_method);

        if ($has_permission != 1) {
            header('Content-Type: text/html; charset="utf-8"');
            switch ($has_permission) {
                case '0':
                    exit("У вас недостаточно прав!");
                    break;
                case '-1':
                    exit("Некорректный пользователь. Обратитесь к программисту!");
                    break;
                case '-2':
                    exit("Данное разрешение отсутствует в списке. Обратитесь к программисту!");
                    break;
            }
        }
...
//        }
    ...
    //проверка доступа
    private function has_permission($class_method)
    {
        $user_id = $this->session->userdata('id');
        return $this->db->query("select industry.has_user_permission({$user_id},'{$class_method}')")->row()->has_user_permission;
    }
	...
	*/
    ...
    function point()
    {
        ...
        $sql = "SELECT
                  counter.*,
                  counter_type.name    as type
                from industry.counter
                join industry.counter_type on counter_type.id = counter.type_id
                  join industry.period ON period.id = industry.current_period_id()
                where counter.type_id = counter_type.id and point_id = " . $this->uri->segment(3);
        $data['query'] = $this->db->query($sql);
        #plomba
	...
	*/
    ...
    function values_sets()
    {
        ...
        $sql = "Select * from industry.counter where id=(select counter_id from industry.values_set where id=" .
            $this->uri->segment(3) . ")";
        if ($this->uri->segment(3) == 'tabtastic.css') {
            die("catched!");
        }
...
//        $this->pdf->Output('example_001.pdf', 'I');
    ...
    {
        #$sql = "select * from industry.report_nds where period_id>={$_POST['start_period_id']} and  period_id <={$_POST['finish_period_id']}";
...
//        $this->load->view('reports/report_to_nalogovaya', $data);
        $this->export_to_excel('reports/report_to_nalogovaya', $data, "отчет_ндс");
    }
...
//        $_POST['is_esf'] = isset($_POST['is_esf'])?1:0;
...
//                $this->pdf->Output('example_001.pdf', 'I');
    ...
    function schetoplata()
    {
        ...
        if ($_POST['type'] == "by_tenge") {
            $tariff_value = $_POST['tariff_valsum_poterue'];
    ...
    function vih_7_re()
    {
        ...
        $data['sql_result'] = $this->db->query($sql);
//        echo "<pre>";
//        var_dump($data['sql_result']->result());
//        echo "</pre>";
//        die();
    ...
    function vih_2_re()
    {
        ...
        if ($_POST['ture_id'] != -1) {
            ...
            $data['sql_result'] = $this->db->get('industry."report_two_re_by_ture"');
        } else {
            $data['ture'] = NULL;
            $this->db->where('period_id', $_POST['period_id']);
            $data['sql_result'] = $this->db->get("industry.report_two_re");
        }
    ...
    function delete_counter()
    {
        ...
        $this->db->delete("industry.counter_plomba");
        #counter_plomba
...
//                }
    ...
    {
        #permission
        $sql = "select * from industry.user where id='{$this->session->userdata('id')}' and password=md5('{$_POST['old_pass']}')";
    ...
    function oplata_svod()
    {
        ...
        $this->export_to_excel('oplata/svod', $data);
...
//            $nach = $this->db->query("select * from industry.nachislenie_v_buhgalteriu");
            for ($i = 1; $i < dbase_numrecords($db) + 1; $i++) {
                dbase_delete_record($db, $i);
            }
            dbase_pack($db);
...
where firm_id={$this->uri->segment(3)} and data_finish is null";
    ...
    public function fine_firm()
    {
        ...
        $this->db->where('firm_id', $firm_id);
        $this->db->where('period_id', $period_id);
        $data['fine_oplata'] = $this->db->get("industry.fine_firm_oplata_itog")->row();
        if (!empty($data['fine_oplata'])) {
            $data['fine_oplata'] = $data['fine_oplata']->fine_oplata_value;
        } else {
            $data['fine_oplata'] = 0;
        }
...
//            $this->export_to_excel("fine/fine_firm", $data);
    ...
    public function pre_diff_tariff_year()
    {
        ...
        $this->left();
        $this->load->view('other_reports/diff_tariff_year/pre', $data);
        $this->load->view('right');
    }
    ...
    public function diff_tariff_year()
    {
        ...
        $this->load->view("other_reports/diff_tariff_year/report", $data);
    }
    ...
    public function pre_analiz_m_year()
    {
        $data['period_years'] = $this->db->get("industry.period_years")->result();
        $this->left();
        $this->load->view('other_reports/analiz_m_year/pre', $data);
        $this->load->view('right');
    }
    ...
    public function analiz_m_year()
    {
        ...
        $this->load->view("other_reports/analiz_m_year/report", $data);
    }
    ...
    public function pre_tenge_year()
    {
        $data['period_years'] = $this->db->get("industry.period_years")->result();
        $this->left();
        $this->load->view("other_reports/tenge_year/pre", $data);
        $this->load->view("right");
    }
    ...
    public function tenge_year()
    {
        ...
        $this->load->view("other_reports/tenge_year/report", $data);
    }

    #конец годовой свод по начислениям


    public function pre_number_of_schet()
    {
        $data['period'] = $this->db->get("industry.selected_period")->result();
        $this->left();
        $this->load->view("other_reports/number_of_schet/pre", $data);
        $this->load->view("right");
    }

    public function number_of_schet()
    {
        $this->db->where('id', $this->input->post('period_id'));
        $data['period_info'] = $this->db->get("industry.period")->row();

        $this->db->where('period_id', $this->input->post('period_id'));
        $data['number_of_schet'] = $this->db->get("industry.number_of_schet")->result();
        $this->load->view("other_reports/number_of_schet/report", $data);
    }


    #годовой свод по оплатам в разрезе фирм
    public function pre_sopfy()
    {
        $data['period_years'] = $this->db->get("industry.period_years")->result();
        $this->left();
    ...
    public function sopfy()
    {
        $data['year'] = $_POST['period_year'];
        $this->db->where('period_year', $_POST['period_year']);
        $data['report'] = $this->db->get("industry.sopfy")->result();
        $this->load->view("other_reports/sopfy/report", $data);
    }

    public function pre_year_saldo()
    {
        $this->db->order_by("dogovor");
        $data['firms'] = $this->db->get("industry.firm")->result();
        $data['period_years'] = $this->db->get("industry.period_years")->result();
        $this->left();
        $this->load->view("other_reports/year_saldo/pre", $data);
        $this->load->view("right");

    }

    public function year_saldo()
    {
        $this->db->where('id', $_POST['firm_id']);
        $data['firm'] = $this->db->get("industry.firm")->row();
        $data['year'] = $_POST['period_year'];
        $this->db->where('period_year', $_POST['period_year']);
        $this->db->where('firm_id', $_POST['firm_id']);
        $data['report'] = $this->db->get("industry.year_saldo")->result();
        $this->load->view("other_reports/year_saldo/report", $data);
    }
    ...
    public function pre_schet_na_oplatu()
    {
        $data['firm_id'] = $this->uri->segment(3);
        $data['period'] = $this->db->get("industry.selected_period")->result();
        $this->left();
        $this->load->view("other_reports/schet_na_oplatu/pre", $data);
        $this->load->view("right");
    }

    public function pre_schet_na_oplatu2()
    {
        $period_id = $_POST['period_id'];
        $firm_id = $this->uri->segment(3);
        $data['firm_id'] = $firm_id;
        ...
        $this->db->where('period_id', $period_id);
        $this->db->where('firm_id', $firm_id);
        $data['sf_date'] = $this->db->get("industry.schetfactura_date")->row();

        $this->left();
        $this->load->view("other_reports/schet_na_oplatu/pre2", $data);
        $this->load->view("right");
    }

    public function schet_na_oplatu()
    {
        $data['org'] = $this->db->get("industry.org_info")->row();

        switch ($_POST['signer']) {
            case '1':
                $data['dir_prof'] = "Уполномоченный подписывать счета фактуры за руководителя согласно приказа №64 от 31.12.2015г.";
                $data['dir_name'] = "Ташкенова С.С.";
                $data['buh_prof'] = "Уполномоченный подписывать счета фактуры за главного бухгалтера согласно приказа №64 от 31.12.2015 г.";
                $data['buh_name'] = $data['dir_name'];
                break;
            case '2':
                $data['dir_prof'] = "Уполномоченный подписывать счета фактуры за руководителя согласно приказа №64 от 31.12.2015г.";
                $data['dir_name'] = "Ламнова Л.П.";
                $data['buh_prof'] = "Уполномоченный подписывать счета фактуры за главного бухгалтера согласно приказа №64 от 31.12.2015 г.";
                $data['buh_name'] = $data['dir_name'];
                break;
            case '3':
                $dir_prof = "Руководитель организации";
                $buh_prof = "Главный бухгалтер организации";
                $dir_name = $data['org_info']->director;
                $buh_name = $data['org_info']->glav_buh;
                break;
            default:
                break;
        }
        ...
        $this->load->view("other_reports/schet_na_oplatu/report", $data);
    }
    ...
    public function pre_firm_year_detailed()
    {
        $data['period_year'] = $this->db->get("industry.period_years")->result();
        $this->db->order_by("dogovor");
        $data['firm'] = $this->db->get("industry.firm")->result();
        $this->left();
        $this->load->view("other_reports/firm_year_detailed/pre", $data);
        $this->load->view("right");
    }

    public function firm_year_detailed()
    {
        $this->db->where("id", $_POST['firm_id']);
        $data['firm'] = $this->db->get("industry.firm")->row();
        ...
        $this->db->where("firm_id", $_POST['firm_id']);
        $this->db->where("period_year", $_POST['period_year']);
        $data['report'] = $this->db->get("industry.firm_year_detailed")->result();
        if (isset($_POST['to_excel'])) {
            $this->export_to_excel("other_reports/firm_year_detailed/report", $data, "Годовая ведомость №{$data['firm']->dogovor} за {$data['period_year']} год");
        } else {
            $this->load->view("other_reports/firm_year_detailed/report", $data);
        }
    }
    ...
    #counter_plomba
    public function add_counter_plomba()
    {
        if (empty($_POST['data']) or (empty($_POST['value']))) {
            exit("Zapolnite polya!");
        }
        ...
        $this->db->where("point_id", $_POST['point_id']);
        $this->db->where("data_finish IS NULL");
        $counter = $this->db->get("industry.counter");

        if ($counter->num_rows() > 1) {
            exit("Na TU neskolko destvuywih schetchikov!");
        }

        $counter = $counter->row();
        $bill_id = $_POST['point_id'];
        unset($_POST['point_id']);
        $_POST['counter_id'] = $counter->id;

        $this->db->insert("industry.counter_plomba", $_POST);
        redirect("billing/point/" . $bill_id);

    }

    public function close_counter_plomba()
    {
        $bill_id = $_POST['point_id'];
        unset($_POST['point_id']);

        if ((($_POST['data_end']) > date("Y-m-d")) || (empty($_POST['data_end']))) {
            exit("Necorrectnaya data!");
        }

        $this->db->where("point_id", $bill_id);
        $this->db->where("data_finish IS NULL");
        $counter = $this->db->get("industry.counter");
        ...
        $this->db->limit(1);
        $this->db->order_by("data", "DESC");
        $this->db->where("counter_id", $counter->id);
        $plomba = $this->db->get("industry.counter_plomba")->row();

        $this->db->where("id", $plomba->id);
        $this->db->update("industry.counter_plomba", $_POST);

        redirect("billing/point/" . $bill_id);
    }

    public function delete_counter_plomba()
    {
        $plomba_id = $this->uri->segment(3);

        $this->db->where("id", $plomba_id);
        $counter_id = $this->db->get("industry.counter_plomba")->row()->counter_id;
        $this->db->where("id", $counter_id);
        $bill_id = $this->db->get("industry.counter")->row()->point_id;
    ...
    //ПРАВА ДОСТУПА-----------------------------------------------------------------------------------------------------
    //главная страница
    public function access_right_index()
    {
        $this->left();
        $this->load->view("access_right/index");
        $this->load->view("right");
    }
    ...
    public function permission()
    {
        ...
        $this->db->order_by("link");
        $data['perm'] = $this->db->get("industry.permission")->result();
        $this->left();
        $this->load->view("access_right/permission", $data);
        $this->load->view("right");
    }

    //редактирование разрешения
    public function edit_permission_data()
    {
        if (isset($_POST['save'])) {
            $perm_id = $_POST['id'];
            unset($_POST['save']);
            ...
            $this->db->update("industry.permission", $_POST);
            redirect("billing/permission");
        }

        $perm_id = $this->uri->segment(3);
        $this->db->where("id", $perm_id);
        $data['perm_info'] = $this->db->get("industry.permission")->row();
        $this->left();
        $this->load->view("access_right/edit_permission_data", $data);
        $this->load->view("right");
    }
    ...
    {
        unset($_POST['add']);
        $this->db->insert("industry.permission", $_POST);
        redirect("billing/permission");
    }
    ...
    public function user_group()
    {
        ...
        $data['user_group'] = $this->db->get("industry.user_group_list")->result();
        $this->left();
        $this->load->view("access_right/user_group", $data);
        $this->load->view("right");
    }

    //редактирование разрешений для группы
    public function edit_group_permission()
    {
        $user_group_id = $this->uri->segment(3);

        $this->db->order_by("has");
        $this->db->order_by("permission_title");
        $this->db->where("user_group_id", $user_group_id);
        $data['user_group_permission'] = $this->db->get("industry.user_group_permission")->result();
        $this->db->where("id", $user_group_id);
        $data['group_info'] = $this->db->get("industry.user_group")->row();
        $this->left();
        $this->load->view("access_right/edit_group_permission", $data);
        $this->load->view("right");
    }

    //добавить группе разрешение
    public function add_group_permission()
    {
        $user_group_id = $this->uri->segment(3);
        $permission_id = $this->uri->segment(4);
        if ($user_group_id && $permission_id) {
            $this->db->insert("industry.group_permission", array("group_id" => $user_group_id, "permission_id" => $permission_id));
            redirect("billing/edit_group_permission/{$user_group_id}");
        } else {
            exit("Access error!");
        }
    }

    //удалить разрешения группы
    public function delete_group_permission()
    {
        ...
        if ($user_group_id && $permission_id) {
            ...
            $this->db->delete("industry.group_permission");
            redirect("billing/edit_group_permission/{$user_group_id}");
        } else {
            exit("Access error!");
        }
    ...
    public function group_user()
    {
        $user_group_id = $this->uri->segment(3);
        $this->db->where("group_id", $user_group_id);
        $data['users'] = $this->db->get("industry.user")->result();

        $this->db->where("id", $user_group_id);
        $data['group_info'] = $this->db->get("industry.user_group")->row();
        $this->left();
        $this->load->view("access_right/group_user", $data);
        $this->load->view("right");
    }
    ...
    public function delete_user()
    {
        ...
        $group_id = $this->db->get("industry.user")->row()->group_id;

        $this->db->where("id", $user_id);
        $this->db->delete("industry.user");
        redirect("billing/group_user/{$group_id}");
    }
    ...
    public function edit_user()
    {
        $user_id = $this->uri->segment(3);
        $this->db->where("id", $user_id);
        $data['user'] = $this->db->get("industry.user")->row();
        $data['user_group'] = $this->db->get("industry.user_group")->result();

        $this->left();
        $this->load->view("access_right/edit_user", $data);
        $this->load->view("right");
    }
    ...
    public function save_user_changes()
    {
        ...
        $this->db->where("id", $user_id);
        $this->db->update("industry.user", $_POST);
        redirect("billing/group_user/{$_POST['group_id']}");
    }

    //стартовая журнала
    public function access_journal_index()
    {
        $this->db->order_by("name");
        $data['user'] = $this->db->get("industry.user")->result();
        $this->left();
        $this->load->view("access_right/journal/index", $data);
        $this->load->view("right");
    }

    //просмотр журнала по определенному пользователю за определенный день
    public function access_journal_day()
    {
        header('Content-Type: text/html; charset="utf-8"');
        ...
        $this->db->where('action_time <=', $action_time_end);
        $this->db->order_by("action_time");
        $data['actions'] = $this->db->get("industry.user_action")->result();
        ...
        $this->left();
        $this->load->view("access_right/journal/view", $data);
        $this->load->view("right");
    }
    //ПРАВА ДОСТУПА-----------------------------------------------------------------------------------------------------
