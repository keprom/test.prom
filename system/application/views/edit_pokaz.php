...
<table>
    <?php foreach ($pokaz->result() as $p): ?>
        ...
        <?php if ($last_bill_id != $p->bill_id): ?>
            <tr>
                <td colspan="5"><?php echo "<h3><b>{$p->bill_name}</b></h3>"; ?></td>
            </tr>
        <?php endif; ?>
	...
	гос. номер <font color=green>" . $p->counter_gos_nomer . "</font></br>  дата установки " . $p->counter_data_start .
        ...
        <?php if ($p->counter_value_value != null): ?>
            <tr>
                ...
                <td><?php echo datetostring($p->counter_value_data) ?></td>
                <td class="td-number"><?php echo f_d($p->counter_value_value) ?></td>
                <td class="td-number"><?php echo f_d($p->counter_value_diff); ?></td>
                <td class="td-number"><?php echo f_d($p->counter_value_diff * $p->transform); ?></td>
