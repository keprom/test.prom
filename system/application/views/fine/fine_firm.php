...
if ($saldo > 0) {
    ...
    echo "Сальдо пени на начало текущего месяца:<b> " . prettify_number($fine_saldo) . ". Оплата пени: ".prettify_number($fine_oplata)."</b><br>";
    ?>
    <table class="border-table" border="1px">
        <thead>
        <tr class="head-tr">
            <th colspan="2">День</th>
            <th>Долг на<br>начало месяца, <br>тнг</th>
            <th>Начисление <br>за <?php echo $pinfo; ?>,<br> тнг</th>
            <th>Ставка<br>реф.(%)</th>
            <th>Коэф.</th>
            <th>Сумма оплаты, <br>тнг</th>
            ...
            <th>Итого пени <br>по начислению <br>за <?php echo $pinfo; ?>, тнг</th>
            <th>ИТОГО <br>ПЕНИ, тнг</th>
        </tr>
        </thead>
        <tbody>
        <?php
        foreach ($firm_veds->oplata as $day => $opl) {
            ...
            if ($dolg_old > 0) {
                $fine_old += ($day - $day_buf_old) * $k * $dolg_old;
            }
            if ($dolg_new > 0) {
                if ($day > $border_day) {
                    $fine_new += ($day - $day_buf_old) * $k * $dolg_new;
                }
            }
    ...
    <?php
    $fine_saldo -= $fine_oplata;
    $fine_total = $fine_new + $fine_old;
    if ($fine_saldo < 0) {
        $fine_total += $fine_saldo;
        if ($fine_total < 0) {
            $fine_total = 0;
        }
    }

    echo "<b>Итого: " . prettify_number($fine_total, 2) . " тенге</b></br>";
} else {
    echo "Пени нет";
}
?>
